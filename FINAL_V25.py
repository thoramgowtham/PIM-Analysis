...
# .........Variables ...........	
market = 'Phase2_last22'        #CHANGE MARKET
...
#debugger log file
debugger = 'C:/Users/me/Desktop/PPTCODES/Source/ExcelFiles/'+ market +'_Debugger.xlsx' #Change debugger excel file
wb1 =load_workbook(debugger)
ws1 = wb1.active

#*******Getting all values from excel************
try:
	wb =load_workbook('C:/Users/me/Desktop/PPTCODES/Source/ExcelFiles/' + market + '_MASTER_V25.xlsx') #change source excel
	ws = wb.active
	...
	#Checking for rfds and siteplans & sitepics	
		if os.path.exists(rfds_path):
			for file2 in os.listdir(rfds_path) :
				...
				temp = file2.split("_")
				#****** Check for RFDS ******
				if temp[1].upper() == 'RFDS.PNG':
					countM = countM+1
					ListM.append(file2)
		   
				try:
					if temp[1].upper() == 'A' and temp[2].upper() == 'RFDS.PNG':
						countM = countM+1
						ListM.append(file2)
					if temp[1].upper() == 'B' and temp[2].upper() == 'RFDS.PNG':
						countM = countM+1
						ListM.append(file2)
				except:
					pass
